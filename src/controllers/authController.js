...
const authController = {
  ...
  register: async (req, res) => {
    try {
      ...
      if (
        ...
        return statusCode.clientError.forbidden(
          res,
          '只有管理员可以创建管理员或版主账户'
        );
      }
      ...
      if (!tokenResult.success) {
        return statusCode.serverError.internalError(
          res,
          '令牌生成失败',
  ...
  updateProfile: async (req, res) => {
    try {
      const { username, email, firstName, lastName, bio, currentPassword, newPassword } =
        req.body;
      ...
      statusCode.success.ok(res, { user: userPublicInfo }, '用户信息更新成功');
    } catch (error) {
      if (error.name === 'SequelizeValidationError') {
        const validationErrors = error.errors.map(err => err.message);
        return statusCode.clientError.badRequest(
