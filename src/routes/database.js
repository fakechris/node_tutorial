...
// 创建示例数据 - 仅管理员
router.post('/sample-data', auth.authenticate, auth.authorize(['admin']), async (req, res) => {
  try {
...
// 清空所有数据 - 仅管理员
router.delete('/clear-all', auth.authenticate, auth.authorize(['admin']), async (req, res) => {
  try {
    const confirmation = req.body.confirm;
    
    if (confirmation !== 'DELETE_ALL_DATA') {
      return statusCode.clientError.badRequest(res, '请在请求体中提供确认字符串: {"confirm": "DELETE_ALL_DATA"}');
    }
...
router.post('/query', auth.authenticate, auth.authorize(['admin']), async (req, res) => {
  try {
    ...
    const queryType = sequelize.QueryTypes[type.toUpperCase()] || sequelize.QueryTypes.SELECT;
...
// 数据库备份信息 - 仅管理员
router.get('/backup-info', auth.authenticate, auth.authorize(['admin']), async (req, res) => {
  try {
    const fs = require('fs');
