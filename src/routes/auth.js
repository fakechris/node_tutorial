...
}), auth.optionalAuth, authController.register);
...
router.put('/profile', auth.authenticate, validateRequest({
  body: {
    username: { 
      required: false, 
      type: 'string', 
...
router.get('/demo/moderator-plus', 
  auth.authenticate, 
  auth.authorize(['admin', 'moderator']), 
  (req, res) => {
    statusCode.success.ok(res, {
      ...
      privileges: req.user.role === 'admin' ? [
        '内容管理',
        '用户管理',
        '系统管理'
      ] : [
...
router.get('/demo/own-resource/:userId', 
  auth.authenticate, 
