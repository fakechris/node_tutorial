...
const performanceMonitor = (req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    // 按方法统计
    const methodStats = performanceMetrics.requests.byMethod.get(req.method) || {
      count: 0,
      totalTime: 0,
    };
    methodStats.count++;
...
const monitorDbQuery = (query, startTime, endTime, result = {}) => {
  ...
  // 保持最新的1000个查询记录
  if (
    performanceMetrics.database.queries.length > performanceMetrics.database.maxQueries
  ) {
...
const getPerformanceStats = () => {
  ...
  const memoryTrend =
    performanceMetrics.memory.samples.length > 1
      ? performanceMetrics.memory.samples[performanceMetrics.memory.samples.length - 1]
          .memory.heapUsed - performanceMetrics.memory.samples[0].memory.heapUsed
      : 0;
